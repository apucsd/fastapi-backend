...
def get_current_user(
    ...
    if not credentials: 
        raise AppException(status_code=401, message="No authorization token provided")
...
def require_role(*roles):
    def checker(current_user: User = Depends(get_current_user)):
        if current_user.role.value not in roles:
            raise AppException(status_code=403, message="You are not authorized to perform this action")
        return current_user
    return checker